...
CEN, HENS, STEPS, R = M.CEN, M.HENS, M.STEPS, M.R
# E093: depletion at its DEFAULT. The whole T2 arc inherited food_deplete_rate=0.0 from
# E082 (E092), and with feeders that never empty nothing ever makes a hen leave one --
# which is the behaviour this experiment measures. peck_stops_walking stays 0.0: E092's
# gate failed its own falsifiers and must not ride along.
CFG = M.CFG._replace(food_deplete_rate=spec.DEFAULT_COOP.food_deplete_rate,
                     peck_stops_walking=0.0)
# E090's adopted anchor, plus E091's scratch suppression (unconditional in the scaffold).
GAKEL_PECK_W, HUNGER_PECK_W = 9.0, 4.0
...
# E095: the gate that decides whether a seed's plant took. Measured LIVE, with the plant
# installed, from the pred_gain=0.0 arm -- the baseline the contrast needs anyway. The
# fit-space numbers stay as diagnostics; they are not the gate, because E093 passed on
# them at 738 654 while the live ratio read 0.91.
LIVE_GATE, MIN_SEEDS = 2.0, 6
...
def build(k):
    return connectome.build(jax.random.fold_in(k, 1), reg, n_hens=HENS,
                            ...
                            gakel_peck_weight=GAKEL_PECK_W,
                            hunger_peck_weight=HUNGER_PECK_W)
...
def _main():
    print(f"E093 -- E089 re-run. depletion {CFG.food_deplete_rate}, "
          f"gakel_peck_weight {GAKEL_PECK_W}, hunger_peck_weight {HUNGER_PECK_W}, "
          f"{SEEDS} seeds, freeze {FREEZE:.0f}s")
    ...
    print("(fit-space numbers above are DIAGNOSTICS, not the gate -- E093 passed on a "
          "mean of 738 654\n while the live ratio read 0.91. The gate is below.)\n")

    # --- E095's gate: live selectivity, per seed, measured with the plant installed.
    # Runs on the pred_gain=0.0 arm, which the contrast needs as its baseline anyway, so
    # this costs nothing. PASSING is decided here, before any behavioural number is read.
    print("live plant gate (E095) -- relu(pred@gakel) at target vs elsewhere, "
          "plant installed")
    print(f"{'seed':>5}{'live @T':>10}{'live elsewhere':>16}{'live ratio':>12}"
          f"{'fit acc':>9}{'fit ratio':>13}{'verdict':>9}")
    PASS, LIVE = [], {}
    pc0 = plasticity.PlasticConfig(**E.FROZEN, pred_gain=0.0, pred_bar_freeze_s=FREEZE)
    for s in range(SEEDS):
        p, tgt, ctl, food, acc_s, ratio_s, _dec = PLANT[s]
        k = jax.random.key(s)
        w = world.reset(k, CFG)._replace(food_pos=jnp.asarray(food, dtype=jnp.float32))
        x = brain.initial_state(p, HENS); ps = plasticity.initial_state(p, HENS, pc0)
        _a, _c, _h, _f, _pk, cnt, gT, gO, cntO = ladder(
            w, x, p, ps, jax.random.fold_in(k, 9), CFG, pc0,
            STEPS // EVERY, EVERY, tgt, ctl)
        vT = float(jnp.sum(gT)) / max(float(jnp.sum(cnt)), 1)
        vO = float(jnp.sum(gO)) / max(float(jnp.sum(cntO)), 1)
        ratio = vT / max(vO, 1e-9)
        ok = ratio >= LIVE_GATE
        LIVE[s] = (vT, vO, ratio, ok)
        if ok:
            PASS.append(s)
        print(f"{s:>5}{vT:>10.3f}{vO:>16.3f}{ratio:>12.2f}{acc_s:>9.1%}"
              f"{ratio_s:>13.2f}{'PASS' if ok else 'fail':>9}")

    _lr = [LIVE[s][2] for s in range(SEEDS)]
    print(f"\nlive ratio: min {min(_lr):.2f}, median {sorted(_lr)[SEEDS//2]:.2f} "
          f"(gate is the MINIMUM, not the mean -- E093 passed on a mean of 738 654)")
    print(f"{len(PASS)} of {SEEDS} seeds pass the live gate: {PASS}")
    if len(PASS) < MIN_SEEDS:
        print(f"\nGATE-VIABILITY FALSIFIER FIRES: {len(PASS)} < {MIN_SEEDS} seeds. "
              f"E085 resolves 5.1% at n=8; below {MIN_SEEDS} the behavioural arm is "
              "underpowered.\nAborting rather than reporting an underpowered contrast.")
        raise SystemExit(0)
    print(f"excluded {SEEDS - len(PASS)} seed(s) on plant quality -- a criterion measured "
          "before\nand independently of the behavioural outcome.\n")
    ...
    _pk0, _pk2 = ROWS[0.0]["pk"], ROWS[2.0]["pk"]
    _supp = (_pk0 - _pk2) / max(_pk0, 1e-9)
    print(f"(behavioural rows above are over {len(PASS)} gate-passing seeds)")
    print(f"anchor    peck@target {_pk0:.3f} -> {_pk2:.3f} = {100*_supp:.1f}% "
          f"suppression (fires if <25%; E089's inert anchor gave 2.9%) -> "
          f"{'FIRES' if _supp < 0.25 else 'clear'}")
