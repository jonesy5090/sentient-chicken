...
**The leading explanation, predicted in advance and unrefuted:** mutation touches only `W`
and `W_out`, while the one demonstrated route from alarm to crouch runs through
`p.reflex` — which is never plastic and never mutated. E027 already found H4's effect
surviving a full `W_out` lesion. Selection was asked to rebuild, in the pathway it can
reach, a benefit only ever shown through one it cannot. **H4 is untouched by this**: it is
a separate claim about the reflex arc.
