...
**The falsifier's own rule was attempted and the attempt is inconclusive.** E097 ran
`W_pred` — the pathway the falsifier names — and its primary fired (muted DiD −0.0236,
wrong-signed). **That verdict is withdrawn on two grounds recorded before it was read.**
The rule was trained through a centred, lagged trace and *tested* through instantaneous
uncentred `rate(x)`, because the assay runs with plasticity off so `z_lag` never updates —
E071's error for the fourth time. And at `hawk_period_s=900`, 30 minutes of rearing
contains roughly **two hawk events**, so the contingency the mechanism needs may never have
been available to learn. A null under either condition is uninformative.
