...
how [E072](experiments/E072-balanced-ei-and-h2d.md)'s `balanced_ei` scored null there
and **2.13× (t=5.75, paired 12 genomes)** under naturalistic input. `balanced_ei` brings
live pallial rate to 0.1209, out of the compressive region for the first time; it is
**not adopted** pending an ethogram re-run, a throughput check, and a check on whether
any existing plastic result depends on the saturated regime. **This is the blocker
...
**Not yet bisected.** H2f's headline result was validated under a control that no longer
holds on current code, and its status should be read with that pending.
