...
**And E098 found something larger than either rule.** The flockmate-to-alarm pairing is
100% at every hawk period, but of 18.3 paired events per run roughly **0.3** occur while the
focal hen is blind -- so **a call carries information she does not already have in ~1.6% of
hawk events.** For the other 98% she is looking at the hawk while being told about it.
...
| E098 | **The audience assay repaired -- and the environment, not the rule, is the problem.** Trace state now advances with plasticity off, and `assay()` carries both the reared `ps` **and** the assay-time `pc`. **My first repair was a no-op and was measured as one** -- arm P came back **8/8 bit-identical to E097**, because `assay()` ran at `NO_PLASTICITY` whose `pred_enabled` is False, so the projection was still sourced from instantaneous `rate(x)` and the moved trace update was skipped by its own gate. Inertness verified bit-identical twice. **Part A: the pairing is 100% at every hawk period** -- every hawk event yields an audible alarm; only arrivals vary (1.0 / 3.7 / 18.3). **But of 18.3 paired events ~0.3 occur while she is blind, so a call carries information she does not already have in ~1.6% of hawk events.** **Part B, fairly:** `W_pred` muted DiD **+0.0815 +/- 0.0359, t=2.27** vs 2.365 -- below the bar, not significant, and **identical to the gain-0 arm (+0.0908)** whose projection never shaped rearing. Primary FIRES; specificity clear; arm H reproduces E096 exactly. **H2f's falsifier is met and not satisfied.** Also: correcting the source drops `alarm_alone` 0.49 -> 0.27, so E097's numbers carried a spurious top-down boost, and the world change moved the result far more than the fix did. |
