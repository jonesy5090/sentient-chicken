...
### The generational route, and what it is waiting on

After six failed mechanistic explanations for H2's null (E100–E115), the route to H0 that
remains does not depend on within-lifetime learning at all: evolve connectomes across
generations, with plasticity off, and give the intact-vs-yoked contrast a transmission
bottleneck — which is what H5 needs anyway.
[E116](experiments/E116-generational-selection.md) built that machinery
(`run/evolve.py`) and showed it produces a replicated behavioural improvement over a
matched unselected control, **but only once standing variation is supplied artificially**.

[E117](experiments/E117-where-standing-variation-comes-from.md) quantified the
prerequisite over 16 genomes and two disjoint seed blocks:

- A flock from `connectome.build` has fitness repeatability **−0.014 ± 0.053** — a hen's
  outcome carries no information about her brain, so no adaptive process can act on
  individual differences. **A flock is not yet a population.**
- The fix is not more anatomy. Giving each hen her own connectivity draw varies **22.65%**
  of her synapses and moves repeatability not at all (+0.055 ± 0.050).
- The fix is **audibility**: amplifying what already differs — either the weights
  (`founder_sigma=2.0`, +0.628 ± 0.090) or the channel they speak through
  (`readout_scale` 0.05→0.25, +0.511 ± 0.101) — works, and the two are equivalent within
  one standard error.
- **The usable band is about one order of magnitude wide.** `readout_scale=1.00` saturates
  the bird — every motor channel pinned near 1.0, zero hens caught in 144 dives — and
  scores +0.881 precisely because being broken is reproducible. Every arm that made hens
  distinguishable also made them worse: **selectability and competence trade off.**
- **Blocking the ladder:** predation repeatability is **zero in all eleven arm-by-block
  measurements**, while the predation term supplies ~74% of `evolve.fitness`'s variance.
  An intact-vs-yoked contrast scored on that criterion spends three-quarters of its
  selection differential on luck, and an alarm channel's whole purpose is predation. The
  criterion or the predator statistics must change before that ladder is worth running.

[E118](experiments/E118-diversity-collapse.md) then asked why the search stalls at
generation 3, and the answer was not the one E116 proposed:

- **Diversity collapse is a correlate, not the cause — the pre-registered falsifier fired
  in both blocks.** Three arms held diversity far above baseline (`parents_8` 17–33%,
  `mut_0.15` 37%, `mut_0.30` **79%**) and not one improved on it
  (−0.0008 t=−0.06; +0.0120 t=+0.78; −0.0556 t=−3.19, *worse*). The arm that did improve
  retained 11–13%, the same collapse as baseline. **The fuel is not what ran out.**
- **The criterion carries it.** Deleting the zero-information predation term
  (`caught_weight` 0.10→0.0) took selected lineages from **4 of 8 improving to 8 of 8**:
  +0.0299 ± 0.0093 (t=3.21) against baseline, absolute +0.0381 ± 0.0079 (t=4.80). Since a
  control never consults fitness, the two arms share their control exactly, so this is a
  fully matched pair.
- **`selected − control` is a misleading statistic across arms** whose mutation rates
  differ, because the control degrades under mutation load. `mut_0.15` tops that ranking
  (t=5.78 pooled) while being level with baseline on the outcome that matters (t=0.78).
  E116 used this statistic and so did E118's own pre-registration.
- **E116's headline does not reproduce at the nearest recoverable configuration**:
  its own `baseline` gives selected−control **+0.0196 ± 0.0183, t=1.07** over 8 lineages,
  4 of 8 improving. E116's coop is unrecoverable (script never committed), so this is
  reported as a failure to reproduce rather than a refutation.
- **Still unexplained:** `drives_only` does 90% of its work by generation 1.5 and still
  ends at 11–13% diversity. Four search fixes are now ruled out and the survivor is
  unnamed.

**[E120](experiments/E120-the-h0-ladder.md) ran the ladder. Status: the generational route
to H0 is TRIED and NOT SUPPORTED — and this is the project's first null that can say what
it excludes.**

24 lineages per arm, `hawk_period_s=20`, 16 generations, founders and worlds matched:

| intact − yoked | estimate | 95% CI | verdict |
|---|---|---|---|
| reduction in catches/dive (primary) | +0.0350 ± 0.0201 | [−0.0066, +0.0765] | null |
| final flock re-assayed, catches/hen | −0.3385 ± 0.3131 | [−0.986, +0.309] | null, 12/24 lineages |
| confound: hunger | +0.0028 ± 0.0093 | — | null |

**Why this null means something.** Before the contrast ran, five things were measured
rather than assumed: the channel varies (0.144 → 0.270), the yoked control destroys 91.3%
of its information, a *planted* comprehension is worth **−1.195 catches/hen**, selection
works on this substrate (E118: 8/8 lineages), and the flock is selectable at all (E117).
The design's per-lineage SD is 1.534, so detecting the planted ceiling needs 13 lineages
and 24 were run — **the ceiling falls outside the 95% CI.** This experiment can exclude an
effect as large as hand-wiring comprehension provides.

**And it nearly went the other way.** Blocks 1–2 (8 lineages, E116's own bar) gave −0.78
and −1.02 in H0's direction. Three of the next four blocks had the wrong sign. E021's
lesson, reproduced.

~~**The leading explanation, predicted in advance and unrefuted:** mutation touches only
`W` and `W_out`, while the one demonstrated route from alarm to crouch runs through
`p.reflex`.~~ **Refuted by [E121](experiments/E121-can-the-evolvable-pathway-see-the-alarm.md),
which replaced it with a better one.**

- **The pathway is dim, not blind.** The alarm decodes at **AUC 0.687** at the motor stub —
  exactly the stage `W_out` reads, by exactly the kind of linear readout `W_out` is. The
  loss happens in the *first* synapse (0.846 → 0.738 → 0.687), matching E103.
- **Legible and still unusable.** Planting that direction into `W_out`'s crouch row buys
  **nothing** over a magnitude-matched scramble — plant − scram alternates sign, all
  |t| < 1.2, with crouch rates matched to three decimals. At d′ ≈ 0.69 the alarm-driven
  swing is smaller than its own noise, so any readout that crouches her when warned crouches
  her when not. **Decodability is not usability** — E081's distinction, at the readout.
- **And the task does not require communication.** On the *scrambled* arm, which carries no
  information by construction, taking crouch 0.399 → 0.790 takes catches 3.375 → 0.930 for
  0.024 hunger: **+0.233 fitness, information-free.** Both ladder arms climb that hill.
  **A communication experiment whose optimal policy is unconditional is not a communication
  experiment**, and the repair is in `coop/` rather than `hen/`.

**[E122](experiments/E122-make-vigilance-cost-something.md) tried to make vigilance
expensive and found it cannot be done with coop parameters — because crouching is
camping.**

Nine settings of patch size × intake rate, none bringing the information-free gain near
zero (it stayed +0.10 to +0.17), with the *drives cost of crouching ≈ 0 in every cell*.
Re-scoring with `n_fed`, the cumulative peck counter `evolve.fitness` never reads, changed
nothing — crouching more leaves total feeding flat and at low bias **raises** it.

The mechanism: `actuation.py` sets `mobility = 1 - crouch`, so a crouching hen stays put.
Measured, raising the crouch bias takes **patch dwell 15.2 → 29.3 chunks (+92%)** and
locomotion −47%, leaving time-at-food unchanged (+1%). She finds patches less often and
works them far longer, and the two cancel.

**So the anti-predator action *is* the best foraging action**, and crouching accidentally
supplies the persistence [E112](experiments/E112-repair-the-peck-reflex.md) named as the
missing foraging behaviour ("what the camped oracle does that the repaired hen still cannot
is stay on its patch"). **There is no vigilance/foraging trade-off in this model to
calibrate**, which is a defect in the *world*, not the brain, the rule, or the channel —
and it is a single fact explaining E120's null, E121's scramble result and E122's failed
calibration at once. E112's foraging gap and E121's communication gap are the same fact
from two directions.

The ladder was **not** re-run: §4's falsifier said not to without a calibrated world, and
that is what stopped E120's meaningless run at `hawk_period_s=10`.

**H4 is untouched by this**: it is a separate claim about the reflex arc.
...
> **Read against [E121](experiments/E121-can-the-evolvable-pathway-see-the-alarm.md) §6d–6e.**
> T1's shape — no intake benefit, a real safety benefit — is what a coop with a *weak*
> vigilance/foraging trade-off would produce by default. Crouching does halve feeding
> while it happens (52.7% of the upright rate, t=−16.71), but a hen is on a patch only
> **4.5%** of the time, so the aggregate cost of vigilance is small and taking crouch
> 0.399 → 0.790 nets **+0.233** fitness. Safety is cheap here and intake has little room
> to move. The E045 result is not overturned; its interpretation should be checked against
> a trade-off much weaker than the many-eyes literature it is measured against assumes.
...
| E122 | **Vigilance cannot be made expensive in this coop, because crouching is camping -- the anti-predator action and the best foraging action are the same action.** E121 found unconditional vigilance worth **+0.233** fitness with no information, making the ladder unwinnable by design; this tried to repair the world, with the calibration target fixed *without reference to the channel* (the net gain from an information-free crouch increase, applied as a constant on `b_motor[M_CROUCH]`). **Nine settings of `peck_radius` x `peck_food_rate`, and none came near zero** -- net stayed **+0.10 to +0.17** at doubled crouch. The informative column was not the net but the *drives cost*, which is **~0 or negative in every cell**: at `peck_radius=0.3, peck_food_rate=5e-3` hunger moves **0.6019 -> 0.6016** while crouch nearly doubles. Patch size and intake rate change hunger's *level* (0.35-0.60 across settings), never its *sensitivity*. **Second suspect, also wrong: the scoreboard.** `evolve.fitness` scores terminal drives against a cumulative catch count, and hunger is a regulated homeostat (intake is `peck_food_rate * hunger`, so a hen who loses feeding time eats faster per second) -- so the foraging side looked compressed by self-correction while predation was not. `coop/world.py` already tracks **`n_fed`**, a cumulative peck counter `fitness` never reads. Re-scoring the same runs with a running total: **n_fed +4.1%, +5.1%, -4.3%** across crouch biases, and the two scorings agree to within noise (+0.034/+0.089/+0.132 against +0.047/+0.082/+0.123). **The foraging cost of vigilance is genuinely zero in aggregate, by both measures.** **The mechanism, measured**: `actuation.py` sets `mobility = 1 - crouch`, so a crouching hen stays where she is. Raising the crouch bias takes **patch dwell 15.2 -> 29.3 chunks (+92%)** and locomotion **-47%**, leaving time-at-food **unchanged (+1%)** -- she finds patches less often and works them far longer, and the two cancel, which is why E121c's real *local* suppression (feeding at 52.7% of the upright rate) produces no aggregate cost. **Crouching accidentally implements the persistence E112 named as the missing foraging behaviour** ("what the camped oracle does that the repaired hen still cannot is stay on its patch"). **An alarm call cannot pay when the behaviour it recommends is what you should be doing anyway** -- one fact explaining E120's ladder null, E121's scramble result and this failed calibration at once, and the first explanation in the sequence that is neither the brain, the rule, nor the channel. **E112's foraging gap and E121's communication gap are the same fact from two directions.** Three predictions, three wrong: patch size and intake were the wrong levers, the scoreboard was the wrong suspect, and the cause was an unexamined coupling between two motor channels. **The ladder was NOT re-run** -- section 4's falsifier said not to without a calibrated world, the same rule that stopped E120's meaningless run at `hawk_period_s=10`. Successors, none tested: give the hen persistence that is not crouching (E112's own owed item, now with a second independent reason); make freezing genuinely exclude feeding, which is what `actuation.py`'s comment already claims; or give crouching a cost that is not foraging. No code changes -- the target was never met, so nothing is adopted. |
| E121 | **E120's null has a mechanism, and it is the task rather than the anatomy: unconditional vigilance is worth +0.233 fitness with no information at all.** E120's leading explanation -- that the alarm never reaches the pathway selection can touch -- was an explanation, not a measurement. `brain.step` computes `cortical = einsum(W_out, motor_stub)`, so `W_out` reads the MOTOR region's rates and nothing else, which makes the claim testable. **Part A, and my prediction was wrong**: decoding "an aerial alarm is audible" among hen-steps where she CANNOT see a hawk herself gives **sensory stub 0.846, pallium 0.738, motor stub 0.687** (pooled AUC, 8 seeds). I predicted below 0.60 at the motor stub. **The evolvable pathway is dim, not blind** -- and 0.687 is not an abstract figure, because `W_out` *is* a pooled linear readout of that stage, the same operation the LDA performs. Almost all the loss is in the FIRST synapse (0.846 -> 0.738), matching E103 from the other side. **Part B: legible and still unusable.** Planting the motor-stub discriminant into `W_out`'s crouch row against a scramble with identical norm, channel and DC treatment -- crouch rates matched to three decimals at every gain (0.430/0.429, 0.455/0.455, 0.546/0.546, 0.660/0.658, 0.796/0.790) -- gives **plant minus scram alternating in sign with every |t| < 1.2**. Catches track *how much* she crouches, never *when*. **Decodability is not usability**: AUC 0.687 is d' ~ 0.69, so the alarm-driven swing is smaller than its own noise, and any readout strong enough to crouch her when warned crouches her when not. E081 named this for the pallium; this is it at the readout. **A first sweep at gains 0.5-4.0 was degenerate and is reported rather than discarded**: catches fell to 8-14% of unplanted with crouch at 0.85-0.89, the scramble capturing nearly the whole benefit -- had only the plant arm run it would have read as "planting comprehension cuts predation by 86%". E117's `readout_1.00` and E118's `mut_0.30` in a third costume. **The real finding**: on the scramble arm, which carries no information by construction, crouch 0.399 -> 0.790 takes catches 3.375 -> 0.930 for 0.024 hunger, a net **+0.233** fitness at E120's calibrated `caught_weight=0.1049`. **Both ladder arms climb that communication-free hill and it dwarfs anything a channel could add. A communication experiment whose optimal policy is unconditional is not a communication experiment.** **And a defect I reported then disproved**: I read `actuation.py`'s "crouching means not foraging" against `world.py`'s `fed = at_food_any & pecking`, concluded a hen could crouch and eat at once, and drafted it as the missing vigilance/foraging trade-off. **Measured, false** -- P(fed | at food, crouching) **0.3632** against **0.6896** upright, **t=-16.71**; a crouching hen feeds at **52.7%** of the upright rate and the comment stands. The reconciliation is a third number: she is at a patch only **4.5%** of the time, so a 47% local penalty lands on a twentieth of her life. **The trade-off is real locally and weak globally.** Consequence: E120's explanation corrected in place; "make the reflex arc heritable" **demoted** (its rationale is now measured false); promoted instead -- raise the *aggregate* cost of vigilance until doubling the crouch rate nets ~0 rather than +0.233, then re-run the ladder. Caveat recorded for T1, whose "no intake benefit, real safety benefit" reads differently once vigilance is known to cost 4.5% of a hen's life. |
| E120 | **The H0 ladder, run at last: an intact channel does NOT beat a yoked one across generations -- and this is the first null in this project that can state what it excludes.** The experiment `run/evolve.py` was built for, reached through E116-E119. **Three pre-checks first, and the important one failed.** At `hawk_period_s=10` (where E119's predation repeatability pointed) the mandatory positive control is null: `auditory_scaffold`, which hand-wires hearing-an-alarm to crouching, saves nobody (**+0.227 +/- 0.503, t=0.45**). Cause is structural -- `hawk_dive_s=12` against a 10 s onset interval means dives OVERLAP and the sky is busy **52.6%** of the time; an alarm that never stops is not a warning. Sweeping density exposes a genuine squeeze -- **selection needs more hawks, the alarm call needs fewer** -- and exactly one density of three threads it: 10 s (r(channel) +0.173, r(caught) +0.478, scaffold t=+0.45 **no help**), **20 s (r(channel) +0.194, r(caught) +0.179, scaffold -1.195 t=-2.82 SAVES HENS)**, 50 s (r(channel) +0.042, r(caught) +0.049, scaffold t=+1.59 no help). 20 s is also the density E026/E030 independently chose for H4. `caught_weight` **recalibrated rather than inherited**: equalising the terms' *repeatable* spread (`sd*sqrt(r)`) gives **0.1049** against E116's 0.10 from raw spreads -- within 5% by a different route. Both arms carry the 4096-step call buffer so only channel *content* differs. **Ladder result, 24 lineages per arm, founders and worlds matched, 16 generations**: primary (reduction in catches/dive) **+0.0350 +/- 0.0201, 95% CI [-0.0066, +0.0765], t=1.74 -- null**, 15/24 lineages favouring intact; final flocks re-assayed on an intact world (the evolved *brain*, not the channel) **-0.3385 +/- 0.3131, CI [-0.986, +0.309], t=-1.08 -- null, 12/24, a coin flip**. Confound checks clean (hunger +0.0028, t=0.30), so nothing general was happening either. **Why the null is informative**: before the contrast ran, the channel was measured varying (0.1445 -> 0.2704), the yoked control measured destroying **91.3%** of its information, a planted comprehension measured at **-1.195 catches/hen**, selection measured working (E118, 8/8 lineages), and the flock measured selectable (E117). Per-lineage sd is **1.534**, so the planted ceiling needs **13** lineages to detect at 80% power and **24** were run -- **the ceiling falls OUTSIDE the 95% CI**. This design excludes an effect as large as hand-wiring comprehension provides. **And it nearly went the other way**: blocks 1-2 (8 lineages, E116's own bar) gave **-0.78 and -1.02** in H0's direction and would have been reported as promising; three of the next four blocks had the **wrong sign**. E021's lesson reproduced, caught only because a power calculation said 8 was not enough. **Leading explanation, predicted in advance and unrefuted**: `_mutate` touches only `W` and `W_out`, while the one demonstrated alarm->crouch route runs through `p.reflex`, which is never plastic and never mutated (E027 found H4's effect surviving a full `W_out` lesion). Selection was asked to rebuild, in the pathway it can reach, a benefit only ever shown through one it cannot. **H4 is untouched** -- a separate claim about the reflex arc. Prediction scored: right that it would be null, wrong to have bounded it at +0.005. Successors: make the reflex arc heritable; decouple `hawk_dive_s` from `hawk_period_s`; plant-then-select (can selection even *retain* a comprehension it is handed?). Standing caveat: being caught still costs a hen nothing physiologically, so predation enters fitness at a weight I chose -- modelling a preference, not an injury. |
| E119 | **Predation IS heritable -- E117's null was one predator density too early, and the H0 ladder is unblocked.** E117 measured predation repeatability at zero across eleven arm-by-block measurements and concluded no criterion could reward hearing about a hawk; E118 then found the working criterion deletes predation entirely. Checked before building on it, and the conclusion was wrong. Whether being caught is a property of the hen depends entirely on how often hawks come: **`hawk_period_s=50` -> 1.75 caught/hen, r=+0.049 +/- 0.093 (t=0.53); 20 -> 2.92, r=+0.179 +/- 0.074 (t=2.46); 10 -> 4.67, r=+0.478 +/- 0.079 (t=6.57)**. Catches are rare events sampled around a hen-specific rate -- at 1.75 events the sampling noise buries the rate, at 4.67 it does not. **Eleven measurements of the same under-powered configuration are one measurement.** Not a general more-events-more-signal artefact: r(-drives) is flat between 20 s and 10 s (0.717, 0.716) while r(caught) nearly triples. **Being caught costs a hen nothing she is scored on**: r(caught, drives) null at every density, matching the source -- `world.py` moves hunger with `at_food_any & pecking` and nothing else, and `n_caught_any` feeds the fitness function and `strike_penalty` but no state variable of the hen. **Predation here is a scoring convention, not an injury.** Prediction that anti-predator behaviour costs foraging was also wrong -- r(crouch, hunger) null everywhere -- so a drives-only criterion is *indifferent* to hawks rather than hostile to them. Two of three predictions wrong, the important one in the direction that unblocks the work. E117 section 6f corrected in place with a pointer; its numbers stand, its generalisation does not. |
| E118 | **The generational search does not stall for want of diversity -- it stalls because the scoreboard cannot see. Pre-registered falsifier fired in both blocks.** E116 read its own plateau as "selection consumes standing variation faster than sigma=0.05 replaces it" and named higher mutation, more parents, or recombination as the fixes. All three were tried, against matched unselected controls, 8 lineages in two disjoint blocks. **Three arms preserved diversity far above baseline** -- `parents_8` 17-33%, `mut_0.15` 37%, `mut_0.30` **79%** against baseline's 11-14% -- **and not one improved on baseline's selected lineages**: -0.0008 (t=-0.06), +0.0120 (t=+0.78), -0.0556 (t=-3.19, *worse*). Recombination (new, `run/evolve.py`, uniform crossover with Dale enforced and a guard test) neither preserved diversity (12.3%) nor helped (+0.0106, t=1.14). **The arm that did improve kept 11-13% diversity -- the same collapse as baseline.** §4's falsifier fires: diversity collapse is a correlate of the plateau, not its cause, and no arm extended the horizon (90% of improvement by generation 1.5-4.3 everywhere). **What worked was E117's correction to the criterion, which touches the search not at all**: deleting the predation term E117 measured at zero repeatability (`caught_weight` 0.10 -> 0.0) took selected lineages from **4 of 8 improving to 8 of 8** -- **+0.0299 +/- 0.0093 (t=3.21)** against baseline, absolute **+0.0381 +/- 0.0079 (t=4.80)**. Because a control never consults fitness, `baseline` and `drives_only` share their control *exactly* (verified, not assumed), making this a fully matched pair. Per block the contrast is +0.0444 (t=4.31) then +0.0155 (t=1.22) -- thinner than the pooled number reads. **A methodological finding that nearly fooled me**: `selected - control` is treacherous across arms with different mutation rates, because the control degrades under mutation load, so a bigger sigma hands selection more damage to repair. `mut_0.15` tops that ranking in both blocks (pooled t=5.78) while being level with baseline on the outcome that matters (t=0.78) and drifting into saturation (forward 0.95). E116 used this statistic; so did this experiment's own pre-registration. **E116's headline does not reproduce** at the nearest configuration recoverable (its script was never committed): its own `baseline` gives selected-control **+0.0196 +/- 0.0183, t=1.07**, 4 of 8 lineages improving. Reported as a failure to reproduce, not a refutation. **Mutation buys diversity by damaging the bird**: `mut_0.30` retains 79% and 0 of 4 lineages improve at all, motor profile forward 0.98 / peck 0.93 / crouch 0.91 -- E117's saturated regime again. Three of four predictions wrong; the one that held (`drives_only`) was the only one derived from a measurement rather than from reasoning about search dynamics. **Still unexplained and now the open question**: `drives_only` does 90% of its work by generation 1.5 and still ends at 11-13% diversity, with four search fixes ruled out and the survivor unnamed. **And a warning for the H0 ladder**: the criterion that makes selection work is indifferent to hawks, and `coop/world.py` attaches no physiological cost to being caught (hunger moves only with `at_food_any & pecking`) -- so an alarm channel may have nothing to be selected for. E119 measures that before the ladder runs. |
| E117 | **Where standing variation comes from: not from anatomy, but from how loudly the individual part of the brain is allowed to speak -- and the usable band is one order of magnitude wide.** E116's binding constraint, answered over 16 genomes in two disjoint blocks, with the error bar E116 lacked. **First, a methodological correction to E116**: it reported single n=16 correlations, where SE(r) ~ 1/sqrt(13) ~ 0.28, so its table of -0.216/+0.052/-0.284/+0.473 is four numbers of which at most one differs from zero. Re-measured across 8 genomes per block: **E116's +0.473 is not refuted** -- block 1 gave +0.105, block 2 gave +0.432, pooled **+0.276 +/- 0.069** -- but the stable quantity is the homeostatic half, where E116's own side-observation was the real finding. **Baseline: a flock from `connectome.build` has r(-drives) = -0.014 +/- 0.053** over 16 genomes. Not a population. **Anatomy is a measured dead end**: per-hen connectivity draws vary **22.65%** of synapses (against 0.00% otherwise) and move repeatability to **+0.055 +/- 0.050**, null in both blocks. **Audibility is the lever**: `founder_sigma=2.0` gives **+0.628 +/- 0.090 (t=8.17)** and `readout_scale` 0.05->0.25 gives **+0.511 +/- 0.101 (t=5.58)** -- perturbing the weights and amplifying the channel they speak through are equivalent within one SE. **The ceiling is close and the falsifier caught it**: `readout_scale=1.00` scores the highest repeatability of any arm (+0.881) with every motor channel pinned near 1.0 (forward 0.99, peck 0.98, crouch 0.97, flee 0.95) and **0 hens caught in 144 dives** against a default flock's 1.44 each -- E027's exact failure, recorded as degenerate, not as a fix. **Selectability and competence trade off**: every arm that made hens distinguishable also raised hunger (-1.126 -> -1.41/-1.47 mean fitness). **Partial dissociation, honestly reported**: a `shared_shift` control reaching the same operating point with between-hen spread untouched at 0.0445 still scores **+0.172 +/- 0.049 (t=3.57)**, about a quarter of `founder2`'s contrast -- so three-quarters is individual differences and a quarter is the operating-point shift, where block 1 alone would have claimed a clean dissociation. **Blocking the H0 ladder**: predation repeatability is not significant in **any of eleven arm-by-block measurements**, while predation supplies ~74% of `evolve.fitness`'s variance -- selection on it spends most of its differential on luck, and an alarm channel exists for predation. **One mechanism withdrawn**: block 1 gave `explore_0.1` a significantly *negative* repeatability (-0.118, t=-3.63), a competition story was built for it, and a depletion-off diagnostic looked confirmatory (-0.118 -> -0.015) -- **block 2 gave +0.138 and pooled it is +0.010**. E021's shape exactly; the mechanism is retracted. Also found while smoke-testing: `connectome.build` draws weights to 1.387 against `plasticity.w_max=0.5`, so ~0.025% are clipped at first consolidation -- pre-existing, confounds nothing, recorded because it means sigma=0 mutation is not the identity. |
| E116 | **Selection across generations produces the first replicated behavioural improvement in this project -- and the reason nothing worked before is that a flock is not a population.** After six failed explanations for H2's null, this changes the mechanism rather than the brain: plasticity off throughout, flocks scored on homeostatic welfare, best hens' connectomes copied with mutation. **The instrument falsifier fired before the experiment ran, and diagnosing it is the finding.** Fitness repeatability -- the SAME connectomes through the SAME world twice, only exploration noise differing -- is **-0.06, -0.22, +0.08** at 300/600/1200 s lifetimes. **A hen's fitness carries no information about her brain**; it is where she happened to be when a hawk came. A first smoke test showed the exact pathology: selected lineages did WORSE than random-parent controls, because selection concentrates luck while mutation degrades a tuned connectome. **Why is not what I expected.** Amplifying between-hen variation distinguishes 'the varying part cannot reach behaviour' from 'the variation is too small', and it is the second: at 2.7x the natural `W_out` spread, fitness repeatability jumps **-0.216 -> +0.473** and hunger repeatability to +0.608. **A flock straight out of `connectome.build` is too genetically uniform for selection to see anything** -- an ordinary population-genetics fact, never checked here because nothing before needed it. Also visible: at natural variation hunger is somewhat repeatable (0.249) while fitness is not, so **the predation term destroys the signal the hunger term carries.** **With standing variation supplied at generation 0, selection works and REPLICATES**: selected lineages improve +0.0243 and +0.0259 while identically-mutated controls DEGRADE, and selected-minus-control is **-0.0359 +/- 0.0119** then **-0.0315 +/- 0.0145** -- both clearing the pre-registered 2xSE bar, though neither clears df=3's brutal 3.182. Pooled post-hoc across 8 lineages: **-0.0341 +/- 0.0092, t=-3.71, df=6**. **The search stalls**: diversity collapses to 12-13% of founding value (degeneracy bar was 10%), and all the improvement happens by generation 3. **This is the same fact as E107/E109/E110 from the population's side** -- all 16 hens share the reflex arc, the arc determines behaviour, and the part that differs between them barely reaches the muscles. **The binding constraint is now the standing variation, not the search**, which is a question about `connectome.build` rather than about learning. Also recorded: my first fitness function used `strike_penalty * n_strike_events`, whose spread is ~1000x the drives' -- E019's 'reward is 87% n_struck' in a new place, caught by a smoke test and recalibrated against `n_caught_any` at a weight that equalises the two spreads. And E111's references are 30-min lifetimes against E116's 10-min: NOT comparable, stated so the script's own printout cannot mislead. |
| E115 | **A structurally faithful subpallium adds nothing. The missing-structure explanation is closed, and it was the best-motivated hypothesis this project has had.** Documenting the model's anatomy against a real chicken's found the subpallium absent entirely -- and the basal ganglia are what a vertebrate uses to SELECT among competing actions, which is the capability E109 (update direction is the arc's own, cosine 0.98), E112 (the residual gap is 'staying put') and E114 (the rule cannot stop at the useful channels) each measured as missing. Built it properly: **striatum 64 and pallidum 32 as populations**, both 100% GABAergic, striatum near-silent at rest (sigmoid(-4.0)=0.018) and pallidum tonically active (sigmoid(+1.5)=0.82), inhibitory striatal collaterals for competition, and the loop pallium -> striatum -| pallidum -| motor, so selection is **disinhibition**. Both regions default to size 0, so N stays 512 and every prior result is untouched; inertness bit-identical 15/15. **Prediction 2 holds** -- the motor stub is materially suppressed by tonic pallidal inhibition, 0.6369 -> 0.4304. **Prediction 3 is wrong.** Motor-stub direction stability is **0.9998 in every arm**: plain, random loop, topographic loop, frozen and learning alike -- identical to what E107 measured, unperturbed by 96 neurons of correctly-wired subpallium by one part in five thousand. Entropy 0.8034 / 0.8019 / 0.8026. Behaviour null everywhere (all |t| < 1.8, six contrasts, Bonferroni ~3.5). **A second build added parallel-channel TOPOGRAPHY**, because a randomly-wired loop cannot select -- releasing one striatal cell disinhibits a scattering of unrelated motor units. It works as a manipulation (pallidal rate 0.6385 -> 0.7534) and changes nothing else. **My manipulation falsifier fired and was mis-specified**: I wrote pallidal >0.7 and striatal <0.15 as RESTING values and measured them in a RUNNING flock, where the pallidum is being inhibited and the striatum driven -- the circuit working, not failing; the resting values are asserted by a unit test and pass. Same class of error as E114's floored metric. **Completes E102's demotion**: building the actual circuit its gate was named after does not reproduce its benefit, so the benefit was never basal-ganglia-like -- the name was doing work the structure was not. **Six explanations for H2's null, six failures**, and this one was different in kind -- about the anatomy rather than the rule -- and failed the same way: the intervention did exactly what it was designed to do, measurably, and behaviour did not follow. Adopted off by default and not recommended; kept because it is correct anatomy the model was missing and the guard tests encode the circuit's properties. |
| E114 | **E102's mechanism is refuted: the gate works by suppressing TURNING, which has nothing to do with vigilance -- and the learned gate is worse than a lesioned version of itself.** E102 claimed 'pecking and turning drive head_down, which blinds her to the sky, so she learned to suppress the behaviours that blind her'. **Reading the source was enough to break half of it**: `HEAD_DOWN_ACTIONS = (M_PECK, M_SCRATCH)`, so turning does not blind her at all, and the gate closes both turn channels while SPARING scratch, which does. **Two of my own designs failed before one worked, both the same way -- manipulating the WORLD when the claim was about the BRAIN.** Setting `head_down_blinds=0.0` sent caught/dive to exactly 0.0000 in both arms, and my primary falsifier fired on a floored metric and printed 'vigilance IS the mechanism' -- not a finding, and CLAUDE.md's rule 5 names it exactly. A graded sweep left only two usable settings, because **head-down blinding dominates predation in this world**: reducing it a quarter more than halves predation (0.1955 -> 0.0875), halving it abolishes it (0.0094). **The third design asked the question where the claim lived** -- open the learned gate's rows selectively, world untouched. **The two turn channels alone carry ~2x the benefit of the entire learned gate** (-0.1413/-0.1451, t=-6.69/-5.15, 183%/198% of the full effect), while PECK alone -- the one closed channel that does blind her -- carries 53%/77%. Replicated on disjoint blocks. So the benefit is a **locomotion** effect: suppressing turning while leaving FORWARD open makes her travel straighter and leave the strike radius. **This explains E113's uniform-arm null rather than contradicting it** -- a flat gate damps FORWARD too, cancelling exactly that advantage, so E113's selectivity finding stands and now has a mechanism; only E102's account of WHAT was selected was wrong. **And a finding about the rule itself**: TURNS-only beats the full learned gate by -0.064/-0.072, because the incidental 1-8% suppression the rule adds to the other nine channels (CROUCH and FLEE included) costs back about half the gain. What learning found is real, structured, and strictly worse than a hand-lesioned version of itself -- a more precise statement of this project's null than 'learning does nothing'. `head_down_blinds` adopted off by default and NOT recommended: the metric floors below 0.75. |
| E113 | **E102's selectivity claim survives its first real control -- and my three predictions against it were all wrong.** E107 showed E101/E102's untrained-brain interaction is empty by construction (W_gate/W_str start at zero, gate sits at sigmoid(4.0)=0.982, so the control arm differs by a 1.8% attenuation), which left the project's only learned-policy claim with no supporting control. E109 made it load-bearing: the multiplicative gate is the one operation here not confined to `dz_motor`'s direction. Two matched controls on the same reared brains -- **permuted** (same learned values, scrambled across channels) and **uniform** (flat, same mean gate) -- mean gate matched within 0.02 in all three blocks. **Three seed blocks were run because blocks 1 and 2 gave OPPOSITE verdicts on the pre-registered primary**, the E021 pattern exactly. **Prediction 1 wrong**: permuted recovers almost none of the benefit (-0.017 pooled, z=-1.10, against the true gate's -0.068) -- scrambling WHICH channel gets which value destroys about three quarters of the effect. **Prediction 2 wrong**: a uniform gate does nothing at all (+0.019, z=+1.13), if anything worse than no gate. **Prediction 3 wrong, and it was the mundane explanation** -- 'mobility = 1 - crouch, so suppressing anything that competes with locomotion helps' predicts the uniform arm should work, and it does not: a blanket suppression that raises average mobility gives nothing, a spiky one does. **True vs permuted: pooled -0.0554 +/- 0.0126, z=-4.40, but the pre-registered per-block bar is met in only 1 of 3 blocks** (t=-0.89, -4.12, -1.72; all three same sign). Pooling was not pre-registered and is recorded post-hoc per E030's precedent. **True vs uniform clears in all three** (z=-7.00). E102's channel signature reproduces exactly a third and fourth time: TURN_R/TURN_L/PECK closed at 0.22-0.45, SCRATCH/FORWARD spared at 0.92-0.95. **So E107 was right about the evidence and wrong about the conclusion** -- the interaction genuinely carried no information, and the claim it supported turns out approximately true anyway, which is why the red-team rule is verification rather than adoption. **Narrowed, not endorsed**: what is established is that a learned, channel-specific assignment beats a scrambled one and a flat one. E102's 'she learned to suppress the behaviours that BLIND her' is not established -- that needs breaking the head_down link with the gate held fixed, and is marked untested. Also fixed mid-experiment: block 1's `none` arm ran with the gate off entirely while reporting 0.982, a 1.8% mismatch that flattered the true arm; blocks 2-3 corrected. |
| E112 | **The innate peck reflex is mis-aimed -- repairing it closes a quarter of the gap, and learning still adds nothing (t=+0.12).** E111 found the hen pecking 39.7% of the time she is standing on food against 62.6% when she is not, because `innate.py` drives `M_PECK` from the two FRONT vision bins only, which is right for approaching a patch and wrong for standing on one -- at 0.30 m the bearing swings every step. Wired `IDX_FOOD_ARRIVAL` (bearing-independent, AUC 0.87-0.99, previously connected to the food CALL and nothing else) to `M_PECK` at weight 4.0, chosen a priori against `REST_BIAS` the way `SCAFFOLD_WEIGHT` is. **My instrument falsifier fired** -- P(peck|on food) reached 75.3% against a bar of 80% -- and the cause is the finding: **median dwell at a patch is ONE STEP.** Half of all on-food episodes last 10 ms, and the arrival pulse is set after that step's motor output is computed, so a one-step visit cannot be pecked at however the reflex is wired. **She walks through the feeder**, which `innate.py:305` already says and which `peck_stops_walking` (E092) has existed for, off by default, ever since. **Post-hoc arm, labelled and on a disjoint seed block**: with both halves P(peck|on food) goes **39.7% -> 97.9%**. **But only 26% of E111's gap closes** (hunger 0.6557 -> 0.5939 against the camped oracle's 0.4223), nothing survives this experiment's own multiplicity threshold (4 contrasts, Bonferroni t~3.5, largest 2.53), and the repair-alone effect does not replicate (-0.0571 t=-2.28, then -0.0360 t=-1.62). **Section 4's interpretation guard, written in advance, resolves to its second branch**: the arc was not the limitation, E111's headroom stands, and the nulls remain about the rule. The uncomfortable outcome -- that ten experiments were asking learning to fix a hand-wiring defect -- did NOT occur. **And learning adds +0.0014, t=+0.12, on the repaired arc**, which is the cleanest statement of H2's null in the project because the baseline has now been repaired to the limit of what its own wiring can express. **The residual three quarters has a name**: the oracle STAYS on its patch and the repaired hen still wanders (at a patch 3.3% vs 4.8%) despite pecking almost perfectly while there. Staying is a policy, not a reflex -- it needs 'I am foraging here' held across seconds -- and E109 showed this rule cannot express a redirection. Adopted off by default despite being a genuine correction, because it moves every baseline and its benefit does not survive correction; any future baseline should set `arrival_peck_weight=4.0` AND `peck_stops_walking=1.0`, since each alone is half a fix. |
